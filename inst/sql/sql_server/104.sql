...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (432851,4112752,4111921,764971,4314337,36716186,200962,36712762,37395835,4164017,4141960)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (432851,4112752,4111921,764971,4314337,36716186,200962,36712762,37395835,4164017,4141960)
  and c.invalid_reason is null
...
WHERE (C.drug_exposure_start_date >= DATEFROMPARTS(2016, 1, 1) and C.drug_exposure_start_date <= DATEFROMPARTS(2022, 11, 1))
...
WHERE (C.observation_date >= DATEFROMPARTS(2016, 1, 1) and C.observation_date <= DATEFROMPARTS(2022, 11, 1))
...
WHERE (C.procedure_date >= DATEFROMPARTS(2016, 1, 1) and C.procedure_date <= DATEFROMPARTS(2022, 11, 1))
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-60,P.START_DATE) AND A.START_DATE <= DATEADD(day,90,P.START_DATE) ) cc 
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,-183,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
