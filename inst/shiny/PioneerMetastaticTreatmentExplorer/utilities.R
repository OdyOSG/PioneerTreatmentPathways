...
.get_timepoints_survsummary <- function(fit, data, times, decimal.place = 0)
{
  ...
  if (is.null(fit$strata)) {
    ...
    fit <- dplyr::mutate(dplyr::group_by(fit,strata), cum.n.censor=cumsum(n.censor))
    n <- aggregate(fit$n.risk, by=list(fit$strata), max)
    strata <- unique(fit$strata)
    strata_names <- as.character(unique(fit$strata))
  ...
  for (st in strata){
    for (time in times){
      row <- fit[fit$time >= time & fit$strata == st, c('strata', 'n.risk', 'cum.n.censor')][1,]
      if (is.na(row[[1]]) & max(times) == time){
        row <- tail(fit[fit$time <= time & fit$strata == st, c('strata', 'n.risk', 'cum.n.censor')], 1)
      }
      row$time <- time
      res <- rbind(res, row)
    }
