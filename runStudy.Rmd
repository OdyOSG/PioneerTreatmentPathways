...
Change the parameters in the code chunck below to match your database. 
This is the only code that should need to be edited to run the study.
...
Create stratification cohorts. There are two types. Bulk strata defined by a csv file and 
strata defined by circe json files.


```{r, eval = F}
# targetStrataXref <- readr::read_csv(here::here("input", "settings", "targetStrataXref.csv"), 
#                                     show_col_types = FALSE)
# 
# bulkStrata <- readr::read_csv(here::here("input", "settings", "bulkStrata.csv"), 
#                               show_col_types = FALSE)
```
...
createAgeStrata <- function(connectionDetails, 
                            upperBound,
                            lowerBound,
                            newCohortId, 
                            targetCohortId, 
  ...
  sql <- glue::glue("
    ...
    WHERE cohort_definition_id = {newCohortId};
    
    INSERT INTO {cohortDatabaseSchema}.{cohortTable}
    SELECT 
      {newCohortId} as cohort_definition_id,
      c.subject_id, 
      c.cohort_start_date, 
      c.cohort_end_date  
    ...
    INNER JOIN {cdmDatabaseSchema}.person p 
      ...
      AND FLOOR(YEAR(c.cohort_start_date) - p.year_of_birth) <= @upperBound
      AND FLOOR(YYEAR(c.cohort_start_date) - p.year_of_birth) >= @lowerBound
...
cohort_counts <- CohortGenerator::getCohortCounts(
    connectionDetails = connectionDetails,
    cohortDatabaseSchema = cohortDatabaseSchema,
    cohortTable = cohortTable) 
  
cohort_counts <- cohortDefinitionSet %>% 
  select(cohortId, name = atlasName, group) %>% 
  left_join(cohort_counts, by = "cohortId") %>% 
...
Use FeatureExtraction to get baseline characteristics for all target cohorts.

```{r}

library(FeatureExtraction)

target_ids <- cohort_counts %>% 
  filter(group == "Target") %>% 
  pull(cohortId)
...
targetStrataXref <- readr::read_csv(here::here("input", "settings", "targetStrataXref.csv"), 
                                    show_col_types = FALSE)
cohortsToCreate <- readr::read_csv(here::here("input", "settings", "cohortsToCreate.csv"), 
                                   show_col_types = FALSE)
...
events <- readr::read_csv(here::here("input", "settings", "TimeToEvent.csv"), show_col_types = F) %>% 
